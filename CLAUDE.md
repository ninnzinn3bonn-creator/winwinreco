...
3 ファイルを読み終えたら、アクティブな残タスクがないことを前提に、ユーザーの依頼内容に沿って作業してください。会議終了後の議事録生成をさらに改善する依頼の場合は、`docs/POST_MEETING_MINUTES_GENERATION_PLAN.md` の「現在の状態」を参照し、未確定仕様を確認してから新規タスク化してください。
...
- **シークレット**: GCP Secret Manager (`groq-api-key` / `elevenlabs-api-key` / `gemini-api-key`)。表面 provider は Groq + ElevenLabs Scribe 固定、Gemini は emergency fallback 用。
...
- STT プロバイダー別 議事録プロンプト (production fixed to ElevenLabs Scribe)
- チャンキング無限ループバグ修正 + 47 テスト追加 (`tests/chunking.test.js`)
- イースターエッグ ミニゲーム (赤テーマ + 「血まみれの目」)

### 残タスク (`docs/TASKS.md`)
- 現在アクティブな残タスクはありません。
- 会議中ストリーミング議事録生成は実装しない方針で確定。
- API キーローテーションは今回は見送り。Gemini fallback は仕様として維持。
- 会議終了後の議事録生成 M1-A / M1-B / M1-C の確定済み範囲は実装済み。chunk 境界 heuristic など品質仕様が未確定の追加改善は、事前確認してから新規タスク化する。
...
3. STT メタデータ (`microphoneDistance` / `recordingDeviceType`) — WebSocket `mic_preset` メッセージで backend へ通知。provider は ElevenLabs Scribe 固定。
...
- **固定 AI/STT プロバイダ / モデル**: 本番 (`cloudbuild.yaml`) は `AI_PROVIDER=groq` (`openai/gpt-oss-120b`)、`STT_PROVIDER=elevenlabs` (`scribe_v2` / realtime `scribe_v2_realtime`)。UI の provider 選択は disabled、API 経由の古い `aiConfig` / `stt_provider` は backend で固定値に正規化する。
